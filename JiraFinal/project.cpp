...
Project::~Project() {
   ...
   for(int k = 0; k < projectLeads.size(); k++) {
	delete projectLeads[k];
   }
   projectLeads.clear();

   for(int p = 0; p < teamDevelopers.size(); p++) {
	delete teamDevelopers[p];
   }
}
//cout operator
...
//test if the given username belongs to this project's team
...
//return the given's username with their role in the project
...
//check if the given username is a member of this project lead
...
//check if the given username is a member of this team developer
        ...
        if(issueID == toDo[i]->getIssueID()) {
         ...
         for(int j = 0; j < inProgress.size(); j++) {
           if(inProgress[j]->findIssue(issueID)) {
                  ...
                  cin>>selection;
                  if(tolower(selection) == 'y') {
                      inProgress[j]->reassignSprintIssue(issueID, assignee);
                      break;
                  }
              }
...
//to update the issue
//check the role of the username --> only allow certains level for certain role
//check where the id issue is in which stage
...
//to move the issue to in progress, completed or cancel
...
//to add the a member to be project leads
//if the given member is in the developer teams, move them to the project lead
//if the given member is not in the team, make them join the team
//if the given memebr is alreadly the project lead, cout the notification
...
//remove a member in the project leads
...
//to add the member to the developer team
//if the member is in the project lead -> move to developer team
//if the member is not in the project, make them join
//if the member is alreadly in the developer team, cout notification
...
//remove the member in the developer team
